...
from mri_degad_preprocessing import get_gad_and_degad_nifti, slice_3d_images
...
def get_data(conf):
    train_a, test_a, train_b, test_b = get_gad_and_degad_nifti(conf['data_root'])
    train_a_out = slice_3d_images(train_a, conf['output_dir_train_a'], "gad")
    test_a_out = slice_3d_images(test_a, conf['output_dir_test_a'], "gad")
    train_b_out = slice_3d_images(train_b, conf['output_dir_train_b'], "nogad")
    test_b_out = slice_3d_images(test_b, conf['output_dir_test_b'], "nogad")

def get_all_data_loaders(conf, train_a, test_a, train_b, test_b):
    print("train a", train_a)
    print("test a", test_a)
    ...
    if 'data_root' in conf:
        train_loader_a = get_data_loader_folder(train_a, batch_size, True,
                                              new_size_a, height, width, num_workers, True)
        test_loader_a = get_data_loader_folder(test_a, batch_size, False,
                                             new_size_a, new_size_a, new_size_a, num_workers, True)
        train_loader_b = get_data_loader_folder(train_b, batch_size, True,
                                              new_size_b, height, width, num_workers, True)
        test_loader_b = get_data_loader_folder(test_b, batch_size, False,
...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
    ...
    transform_list = [PadToSize((width, height)),
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
    ...
    transform = transforms.Compose(transform_list)
    # this line failing - look at input folder 
